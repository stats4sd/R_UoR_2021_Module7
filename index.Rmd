...
census_data<-data.frame(born=sample(c(rep("UK",48571345),rep("Outside of the UK",7505108))))
...
I've limited the output to just 100 rows to prevent this all showing everywhere!
...
This time you should see a very similar looking distribution of values - but notice that the x axis will have shrunk considerably. 

Also notice in the code we are using 500 in two different contexts - 500 iterations of samples of size 500. In the first line and in the `for` line we are referring to the number of iterations. In the `slice` line we are referring to the number of samples.

All of our estimated values are in a much narrower range. Perhaps we want to look at the range in which 95% of our values lie? We could use the `quantile`  function and take a look at the 2.5th and 97.5th percentiles - so that the 'middle' 95% of the data will lie within this range.
...
We can plug that in to the rest of that formula and see if the theoretical confidence interval matches the simulation 
...
```
We can see this new function works exactly in the same way if we don't trigger an error:
```{r qs2,exercise=TRUE}
quad_solve2(a=100,b=10,c=-5)

```
But if we do trigger that error we now have our lovely new message to help us interpret what has gone wrong:
```{r qs3,exercise=TRUE}
quad_solve2(a="One hundred",b=10,c=-5)
```
